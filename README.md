...
| Embedded self-call splicing + update-aware re-boxing (v1.6 rewrite-correctness fixes) | Found by *benchmarking* v1.6's own newly-qualifying real functions (extracted verbatim into `benchmarks/`, not just gate-checked in place): a self-call to the qualifying function's own name/arity embedded inside a base clause's body (e.g. one branch of an intra-clause `case` that recurses, `xml_vsn/4`'s own shape) now gets its accumulator argument spliced too (`rewrite_nonrecursive_multi/6`, reusing `splice_entry_args/3`, deliberately scoped to base clauses only - a recursive clause's own tail call is already correctly rewritten, and re-processing it generically is unsound whenever the accumulator has exactly 1 field, since old and new arity then coincide); `subst_bare_return/5` now reconstructs an update expression using the accumulator as its own base directly, instead of falling through to generic recursion and producing a redundant, splice-defeating full reconstruction |
...
0 before it. All 4 newly-qualifying real functions are verified to a
stronger bar than "qualifies" too: each one compiles, runs, and
produces bit-identical output to its untransformed original inside a
dedicated benchmark module (`benchmarks/README.md`) - which is how the
two rewrite-correctness fixes above were found in the first place (see
`corpus-study/README.md`'s own methodology-gap note: the corpus study's
"qualifies" signal only ever checked arity change, never
compilability).
...
- `benchmarks/` - 18 benchmarks: 14 from the paper's Table 1, ported from FOL's `benchmarks/fol-code/asr-*.fol`, plus 4 real functions extracted verbatim from Erlang/OTP (the corpus study's own v1.6-unlocked loops); see `benchmarks/README.md` for results and how to run them
