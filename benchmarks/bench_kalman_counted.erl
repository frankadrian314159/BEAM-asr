-module(bench_kalman_counted).
-export([run/1]).
-record(kstate, {x, v}).
-record(kcov, {p00, p01, p11}).

run(N) ->
    bench_util:reset_counter(kstate),
    bench_util:reset_counter(kcov),
    bench_util:bump(kstate),
    bench_util:bump(kcov),
    loop(#kstate{x = 0.0, v = 0.0}, #kcov{p00 = 1.0, p01 = 0.0, p11 = 1.0}, 0, N).
